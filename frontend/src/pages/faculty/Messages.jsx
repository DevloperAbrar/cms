...
const FacultyMessages = () => {
  ...
  const { data: messages = [], isLoading } = useQuery({
    ...
    queryFn: facultyApi.getMessages,
  });

  const { data: students = [] } = useQuery({
    queryKey: ['faculty-students-all'],
    queryFn: () => facultyApi.getMyStudents({}),
  });

  const send = useMutation({
    mutationFn: facultyApi.sendMessage,
    onSuccess: () => { qc.invalidateQueries(['faculty-messages']); toast.success('Sent'); setModal(false); reset(); },
