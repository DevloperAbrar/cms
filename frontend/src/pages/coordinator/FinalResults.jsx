...
import { Save, Globe, EyeOff, BarChart2, List, ClipboardList, TrendingUp } from 'lucide-react';
import {
...
const StatsBar = ({ students, config, valuesMap }) => {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-4">
      {[
        { label: 'Filled', value: filled.length, sub: `/ ${students.length}`, color: 'text-gray-700' },
        ...
        <div key={s.label} className="bg-gray-50 rounded-xl p-3 text-center border border-gray-100">
          <p className={`text-lg font-bold ${s.color}`}>{s.value}<span className="text-xs font-normal text-gray-400">{s.sub}</span></p>
...
const AssignMarksTab = () => {
  ...
  const { data: configs = [], isLoading: configsLoading } = useQuery({
    queryKey: ['coord-final-configs'],
    queryFn: coordinatorApi.getFinalResultConfigs,
  });

  const ready = !!(selectedConfig && selectedBranch);

  const { data: studentsData, isLoading: studentsLoading } = useQuery({
    queryKey: ['coord-final-students', selectedConfig, selectedBranch],
    queryFn: () => coordinatorApi.getFinalResultStudents({ config_id: selectedConfig, branch_id: selectedBranch }),
    enabled: ready,
    onSuccess: (data) => {
      const map = {};
  ...
  const submitMutation = useMutation({
    mutationFn: coordinatorApi.submitFinalResults,
    onSuccess: () => {
      toast.success('Results saved');
      qc.invalidateQueries({ queryKey: ['coord-final-students', selectedConfig, selectedBranch] });
    },
    onError: (e) => toast.error(e.response?.data?.message || e.message),
  });

  const publishMutation = useMutation({
    mutationFn: coordinatorApi.publishFinalResults,
    onSuccess: () => {
      toast.success('Results published — students can now see their results');
      qc.invalidateQueries({ queryKey: ['coord-final-students', selectedConfig, selectedBranch] });
    },
    onError: (e) => toast.error(e.response?.data?.message || e.message),
  });
  ...
  const unpublishMutation = useMutation({
    mutationFn: coordinatorApi.unpublishFinalResults,
    onSuccess: () => {
  ...
  const handleUnpublish = () => {
    unpublishMutation.mutate({ config_id: selectedConfig, branch_id: selectedBranch });
  };

  const getValue = (s) => valuesMap[s._id] ?? (s.result?.value != null ? String(s.result.value) : '');
  ...
  return (
    <div className="space-y-5">
      ...
      {!ready && <EmptyState title="Select a config above to begin" />}
      {ready && studentsLoading && <LoadingSpinner />}
      {ready && !studentsLoading && students.length === 0 && (
        <EmptyState title="No students found" description="No active students match this config's year and semester." />
      )}
        ...
        <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {distData.length > 0 && (
                <div className="card p-5">
                  ...
                  <p className="text-xs text-gray-400 mb-3">Spread of scores entered so far</p>
                    ...
                    <BarChart data={distData} barCategoryGap="20%">
                      ...
                      <ReferenceLine
                        x={distData.find((b) => {
                          ...
                          return lo <= (config?.passing_value ?? 0) && (config?.passing_value ?? 0) <= hi;
                        })?.range}
          ...
          <div className="card">
            ...
            {viewMode === 'chart' && filledStudents.length > 0 ? (
              <div className="p-5">
                <ResponsiveContainer width="100%" height={Math.max(students.length * 36, 200)}>
                  <BarChart
                    data={students
                      .filter((s) => getValue(s) !== '')
                      .map((s) => ({
                        ...
                        passed: Number(getValue(s)) >= (config?.passing_value ?? 0),
                      }))
                  ...
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">
                        {config?.label} <span className="text-gray-400">/ {config?.max_value}</span>
                      </th>
                          ...
                          <td className="px-4 py-2">
                            {isPublished ? (
                              ...
                              <input
                                ...
                                value={val}
                                onChange={(e) => setValuesMap((p) => ({ ...p, [s._id]: e.target.value }))}
                              />
                            )}
                          </td>
            ...
            <div className="px-5 py-4 border-t border-gray-100 flex gap-3 justify-end">
              ...
              {!isPublished && (
                <button
                  ...
                  disabled={publishMutation.isPending || !hasAnyResult}
                >
...
// ─── Analytics & Rankings Tab ─────────────────────────────────────────────────
const AnalyticsTab = () => {
  const { data: branchData, isLoading } = useQuery({
    queryKey: ['coord-final-results-branches'],
    queryFn: coordinatorApi.getFinalResultBranches,
  });
...
// ─── Main Component ───────────────────────────────────────────────────────────
const CoordinatorFinalResults = () => {
  const [activeTab, setActiveTab] = useState('assign');

  const tabs = [
    { id: 'assign', label: 'Assign Marks', icon: ClipboardList },
    { id: 'analytics', label: 'Analytics & Rankings', icon: TrendingUp },
  ];
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Final Results"
        description="Enter and publish results, or view institute-wide analytics and rankings"
      />
      ...
      {activeTab === 'assign' && <AssignMarksTab />}
      {activeTab === 'analytics' && <AnalyticsTab />}
    </div>
  );
};

export default CoordinatorFinalResults;